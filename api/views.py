...
def product_detail(request,product_id):
    try:
        ...
        return Response({'mensaje':'El codigo {} no tiene producto asociado.'.format(product_id)},status=status.HTTP_404_NOT_FOUND)
    ...
    if request.method == 'GET':
        ...
        producto.delete()
        return Response({'mensaje':'Se ha eliminado el producto asociado al codigo {}'.format(product_id)},status=status.HTTP_204_NO_CONTENT)


@api_view(['GET','POST'])
def product_marca(request):
    if request.method == 'GET':
